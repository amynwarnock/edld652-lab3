...
ggplot(total_prod, aes(year, state_complete)) +
  geom_tile(aes(fill = totalprod)) +
  labs(y = "U.S. States", 
       fill = "Total production, in millions of lbs.",
  ...
  #scale_y_discrete(limits = rev(levels(total_prod$state_complete))) +
  theme(axis.text.y = element_text(color = label_color,face = label_face))+
   scale_fill_viridis_c(option = "magma")
```
I tried using the heatmap next. 
```{r part1-heatmap}
str(honey_full)
ggplot(honey_full, aes(state, year, fill = totalprod))+
  ...
  #scale_fill_brewer(palette = "YlOrRd") +
    ...
    guides(fill = guide_legend(title = "Total Production, in millions of lbs.",
...
```
Another way to vizualize the change over time with highlighting the western states, is to have a line plot, with all the states besides Western ones being in the background. Below are versions with 3 Western states together, and with each of them faceted separately. 
```{r part1-line }
...
```
I also made a line plot for each state. Besides highlighting the line for each Western state by different color, I tried using the Label of the state printed differently as well. However, ggplot was not accepting a vector of colors for 'element_text()', and none of the troubleshooting helped.
```{r part1-facets}
states <- unique(total_prod$state_complete)
label_color <- ifelse(states == "Oregon", "#03b800",
                      ifelse(states == "Washington", "#4714ff", 
...
```
I have also tried creating the dumbbell plot to show the difference between 1998 and 2012 in honey production. That plot is not ideal, since it only shows the change between the initial and final points, without reflecting any fluctuations in between. 
```{r part1-dumbbell}
dumbbell <- total_prod %>% 
  mutate(year = as.character(year), 
         state_complete = as.factor(state_complete))

#dumbbell$state_complete <- factor(dumbbell$state_complete, levels = rev(levels(dumbbell$state_complete)))
